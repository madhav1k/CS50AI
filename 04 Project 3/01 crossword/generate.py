...
class CrosswordCreator():
    ...
    def overlap_satisfied(self, x, y, val_x, val_y):
        """
        Helper function that returns true if overlapping squares 
        satisfy the constraint of having equal values i.e. 
        val_x and val_y satisfy any overlap arc consistency requirement 
        for variables x and y.

        Inputs:
            x: variable x
            y: variable y
            val_x: a string value from variable x's domain
            val_y: a string value from variable y's domain
        ...
        while arcs:
            ...
            if self.revise(var_x, var_y):
                ...
                if not self.domains[var_x]:
                    return False
                # If revised, add to arcs all neighbors of x
    ...
    def consistent(self, assignment):
        ...
        for var_x in assignment:
            ...
            # Check if variable is assigned its correct length
            ...
            for var_y in self.crossword.neighbors(var_x):
                # Check if neighbor variable is assigned
                if var_y in assignment:
                    val_y = assignment[var_y]

                    # Return False if neighbor variable conflicts i.e. doesn't satisfy constraints
    ...
    def order_domain_values(self, var, assignment):
        ...
        for val in self.domains[var]:
            # Iterate through unassigned neighboring variables and values:
            for other_var in self.crossword.neighbors(var) - set(assignment.keys()):
        ...
        # SIMPLE BUT INEFFICIENT - RETURN IN ANY ORDER:
    ...
    def select_unassigned_variable(self, assignment):
        ...
        # SIMPLE BUT INEFFICIENT - RETURN ANY VARIABLE:
